...
class SCD(object):
    ...
    def train(self, data, labels, val_data=None, val_labels=None,
        ...
        # results = []
        temp_w = np.random.uniform(-1, 1, size=(num_cols,)).astype(
            np.float32)
        temp_b = 0
        for r in range(self.round):
            if warm_start and self.w_index != []:
                ...
                temp_w, temp_b, temp_obj = self.single_run_adv(train_data,
                                      train_labels, plus, minus,
                                      val_data, val_labels, temp_w, temp_b,
    ...
    def single_run_adv(self, data, labels, plus, minus, val_data, val_labels, w,
        ...
        # L2 normalization
        temp_w = w / w.norm()
        data_adv = self.get_adv(data, w, b)
        temp_b = 0
        # self.w_inc = self.inc_scale * self.step
        for i in range(self.num_iters):
            ...
            row_index2 = row_index.copy()
            np.random.shuffle(row_index2)
            row_index2 = row_index2[:int(row_index.shape[0]*0.75)]
            train_data = torch.cat([data[row_index2], data_adv[row_index2]], dim=0)
            train_labels = torch.cat([labels[row_index2], labels[row_index2]], dim=0)

            temp_w, temp_b = self.single_iteration(
                temp_w, train_data, train_labels.cuda(self.device), plus, minus)

            temp_acc = self.eval(torch.cat([data[row_index], data_adv[row_index]]),
                                 torch.cat([labels[row_index], labels[row_index]], dim=0).cuda(self.device),
                                 temp_w, temp_b, batch_size=None)
