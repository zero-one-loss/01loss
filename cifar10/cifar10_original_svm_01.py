...
if __name__ == '__main__':
    ...
    with open('../results/cifar10/%s_%s_%s_%d.csv' %
    ...
    test_adv = get_adv(model=substitute_model, x=test_data, y=test_label, epsilon=param['epsilon'])
    # train_adv = torch.cat([train_data, train_adv]).squeeze(1).numpy().reshape((-1, 32*32*3))
    # train_labels = torch.cat([train_label, train_label]).numpy()
    #
    # test_adv = torch.cat([test_data, test_adv]).squeeze(1).numpy().reshape((-1, 32*32*3))
    # test_labels = torch.cat([test_label, test_label]).numpy()
    #
    os.chdir('cifar10_adv_data')
